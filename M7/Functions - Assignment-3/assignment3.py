...
# your code locally so that the payment can be any dollar
...
# ultimately pay must be greater than what we would've paid in
# monthly installments, because the interest was compounded on the balance
...
def calci(month, balance, minbal, monthlyintrestrate):
    '''Assig - 3'''
    while month < 12:
        unpaidbal = balance - minbal
        balance = unpaidbal + (monthlyintrestrate * unpaidbal)
        month += 1
    return balance
...
def payingdebtoffinayear(balance, annualinterestrate):
    ...
    monthlypaymentupperbound = (balance * ((1.0 + monthlyintrestrate)**12)) / 12.0
    epsilon = 0.01
    minbal = (monthlypaymentlowerbound + monthlypaymentupperbound) / 2.0
    month = 0
    while abs(balance) >= epsilon:
        balance = newbalance
        month = 0
        balance = calci(month, balance, minbal, monthlyintrestrate)
        if balance >= 0:
            monthlypaymentlowerbound = minbal
        else:
            monthlypaymentupperbound = minbal
        minbal = (monthlypaymentlowerbound + monthlypaymentupperbound) / 2.0
    minbal = round(minbal, 2)
    return str(minbal)
...
def main():
    ...
    print("Lower payment:", payingdebtoffinayear(data[0], data[1]))
